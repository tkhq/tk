...
fn encode_request_identities_response_matches_expected_frame() {
    ...
    let frame = protocol::encode_request_identities_response(&public_key_blob).unwrap();
...
fn parse_sign_request_frame_extracts_key_blob_payload_and_flags() {
    ...
    let request = protocol::parse_sign_request_frame(&frame).unwrap();
...
fn encode_sign_response_matches_expected_frame() {
    ...
    let frame = protocol::encode_sign_response(&signature).unwrap();
...
fn parse_sign_request_frame_rejects_malformed_or_truncated_frames() {
    ...
    let truncated_error = protocol::parse_sign_request_frame(truncated).unwrap_err();
    assert_eq!(
    ...
    let unsupported_error = protocol::parse_sign_request_frame(&unsupported).unwrap_err();
    assert_eq!(
