...
Experimental Turnkey CLI focused on general agent authorization, attribution, and credential management.
...
Use `tk` as Git's SSH signing program after configuring your Turnkey credentials. The full setup guide lives in [Git signing](docs/git-signing.md).

## SSH signing

Use `tk` as an SSH agent for signing SSH requests. Find the setup guide [here](docs/ssh-agent.md).
