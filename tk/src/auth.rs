//! Identity selection for API commands.
//!
//! Identities come from exactly one source per invocation: a complete
//! `TURNKEY_*` environment bundle or a named profile in the identity registry.
//! Credential secrets are never accepted as command-line arguments.
...
/// A stored API credential file. Private keys never leave this file.
#[derive(Serialize, Deserialize)]
pub struct StoredApiKey {
    /// Hex-encoded compressed P256 public key.
    pub public_key: String,
    /// Hex-encoded P256 private scalar.
    pub private_key: String,
    /// Credential curve; only P256 is supported.
    pub curve: KeyCurve,
}

/// Supported credential curves.
...
/// The registry's version alone, read before the strict schema so a newer
/// file reports its version instead of an unknown-field parse failure.
#[derive(Deserialize)]
...
#[cfg(test)]
impl ResolvedAuth {
    /// A directly assembled identity for unit tests.
...
/// Builds a typed Turnkey API client for a resolved identity.
///
/// Redirects are never followed: a redirected request would replay the stamp
/// and body to another host, and query stamps carry no timestamp.
...
/// An auth or profile command result.
#[derive(Serialize)]
...
async fn load(path: &Path) -> Result<Registry> {
    ...
    toml::from_str(&text).map_err(|_| invalid().into())
}

/// Serializes mutations of a protected file so concurrent agents cannot
/// silently lose state.
///
/// The lock is an advisory `flock` on a small lock file, so the kernel releases
/// it when the holding process exits for any reason; a crash never leaves a
/// stale lock behind. The file itself is left in place.
pub(crate) struct FileLock {
    _file: fs::File,
}

/// Another process holds the lock on a protected file.
...
impl FileLock {
    /// Takes `lock`; `resource` names the protected resource in the error when
...
/// Why [`secure_create`] could not create its file.
#[derive(Debug, thiserror::Error)]
pub enum SecureCreateError {
    /// Something already exists at the path (including a symlink); nothing was
    /// touched.
...
/// Creates a new 0600 file without following symlinks or overwriting anything.
...
fn parse_key(private: &str, public: &str) -> Result<TurnkeyP256ApiKey> {
    // The hex decoder's error names the offending character; never echo a
    // character of the private key back to the user.
...
pub async fn run_auth(command: AuthCommand, options: &AuthOptions) -> Result<AuthOutput> {
    match command {
        ...
        AuthCommand::Logout => {
            ...
            // Presence mirrors `resolve`: a set but empty variable still
            // triggers the environment path.
        ...
        AuthCommand::Login(args) => {
            ...
            // Verify remotely before taking the lock so a slow or hung request
            // never holds other writers.
