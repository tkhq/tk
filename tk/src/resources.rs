//! User, credential, and policy operations with typed request preparation.
...
impl BodyArgs {
    /// Reads and validates the parameters object. Every failure here is a
    /// local input problem, classified as `invalid_input`.
...
/// A semantic check on already parsed parameters.
...
impl PreparedResource {
    ...
    async fn run_with_credentials(
        ...
        // Mutations share the versioned activity envelope around their typed,
        // locally validated parameters.
        macro_rules! submit_operation {
            ($params:expr, $kind:literal, $endpoint:literal) => {
                submit(
                    command,
        ...
        macro_rules! client {
            () => {
                crate::auth::build_turnkey_client(stamper, &api_base_url)?
            };
        }
        let response = match self.operation {
            ...
            Operation::CreateUsers(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_CREATE_USERS_V4", "create_users")
            }
            Operation::UpdateUser(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_UPDATE_USER", "update_user")
            }
            Operation::DeleteUsers(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_DELETE_USERS", "delete_users")
            }
            ...
            Operation::CreateTag(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_CREATE_USER_TAG", "create_user_tag")
            }
            Operation::UpdateTag(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_UPDATE_USER_TAG", "update_user_tag")
            }
            Operation::DeleteTags(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_DELETE_USER_TAGS", "delete_user_tags")
            }
            ...
            Operation::CreatePolicy(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_CREATE_POLICY_V3", "create_policy")
            }
            Operation::CreatePolicies(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_CREATE_POLICIES", "create_policies")
            }
            Operation::UpdatePolicy(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_UPDATE_POLICY_V2", "update_policy")
            }
            Operation::DeletePolicy(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_DELETE_POLICY", "delete_policy")
            }
            Operation::DeletePolicies(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_DELETE_POLICIES", "delete_policies")
            }
            ...
            Operation::DeleteKeys(params) => {
                submit_operation!(params, "ACTIVITY_TYPE_DELETE_API_KEYS", "delete_api_keys")
            }
        };
        Ok(response)
    }
        ...
        for (args, endpoint, kind) in cases {
            for status in [
                ...
                let result = prepare(&args)
                    ...
                    .await;
                // Rejected and failed activities are errors that still carry
                // the observed identity; everything else is a record.
                match result {
                    Ok(output) => {
                        assert!(!matches!(
                            status,
                ...
                if endpoint == "update_policy" {
                    assert_eq!(
                        ...
                        json!({
                            ...
                            "policyCondition": "true", "policyConsensus": null, "policyNotes": null
                        })
                    );
                }
                server.verify().await;
